<template>
  <div id="logIn" class="background">
    ...
    <div class="logon">
      <!-- 左边部分 -->
      <img src="../static/leftPic.png" class="letfPic" id="letfPic" />
      <!-- 右边部分 -->
      <div style="overflow: hidden" id="rightFrom" class="fan">
        ...
        <div class="rightFrom" id="rightFrom">
          <!-- 登录头部 -->
          <div class="denglTop">
            <div class="denglTopOne" @click="deng">
              <p class="yanz">微信登录</p>
            </div>
            <!-- 三角形遮挡 -->
            <div class="zhedangDuan" @click="deng"></div>
    ...
    <div class="zuoxia" id="zuoxia">
      <img src="../static/zuoxia.png" />
    </div>
    <!-- 右上角 -->
    <div class="youshang" id="youshang">
      <img src="../static/youshang.png" />
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    logon() {
      ...
      } else if (!this.duanxing) {
        this.$message("请输入验证码");
      } else {
        request
          .post(
            "login/",
            qs.stringify({ phone: this.phone, messages: this.duanxing })
          )
          .then((res) => {
            console.log(res.data);
            if (res.data == "验证码错误或超时！") {
              this.$message("验证码错误！");
            } else {
              ...
              this.$store.commit(
                "setCounty",
                jwt.decode(sessionStorage.getItem("token")).sector
              );

              // 存放token
              this.$store.commit(
                "setRouteObj",
                JSON.stringify(res.data.interfaces)
              );
              genRoute(store.state.routeObj);
    ...
    yanz() {
      if (!this.phone) {
        ...
        if (this.tin) {
          ...
          // 向后端获取验证码
          request
            .post("verification/", qs.stringify({ phone: this.phone }))
            .then((res) => {
              console.log(res);
              this.$message({
                message: res.data,
      ...
      // request
      //   .post(
      //     "http://weixin.cmtthz.com/Mgck/sms",
      //     qs.stringify({
      //       phone: 15957135663,
      //       vcode: this.sssb + this.sss,
      //     })
      //   )
      //   .then((re) => {
...
.logon {
  ...
  border-image: url(../static/di.png) 30 30 stretch; /* Safari 5 and older */
  -o-border-image: url(../static/di.png) 30 30 stretch; /* Opera */
  border-image: url(../static/di.png) 30 30 stretch;
  ...
  .rightFrom {
    ...
    .denglTop {
      ...
      .denglTopTwo {
        width: 56px;
        height: 47px;
        background-image: url(../static/验证码.png);
      ...
      .denglTopTwoDuan {
        width: 56px;
        height: 47px;
        background-image: url(../static/扫码.png);
        background-size: 56px 47px;
        margin-right: 30px;
      }
    }
