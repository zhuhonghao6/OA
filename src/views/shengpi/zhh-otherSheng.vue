...
export default {
  ...
  created() {
    ...
    gets("ApprovalObj/getApprovalObjById", {
  ...
  methods: {
    ...
    tij() {
      ...
      if (this.location == 0) {
        if (this.form2.desc == "") {
          this.form.approvalComments = "无意见";
        } else {
          this.form.approvalComments = this.form2.desc;
        }
      } else {
        if (this.form2.desc == "") {
      ...
      this.form.approvalStatus = this.approvalProgress.toString();
      // this.shuj();

      postp("/ApprovalObj/updateApprovalObj", this.form)
        .then((res) => {
          console.log(res);
          // 按钮
          setTimeout(() => {
